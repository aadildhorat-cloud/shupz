/**
🧴 Shupz - Centralized Product Data & Utilities (ULTRA PERFORMANCE EDITION)
📁 Path: /shupz/js/products-shupz.js
✅ FIXED: Removed all syntax errors, fixed template literals, and exposed window.SHUPZ_PRODUCTS.
*/
(function () {
'use strict';
...
SHEETS_API_URL: "https://script.google.com/macros/s/AKfycbwzFvNT4snhc6vNcHEKCkfxax990TEYtsqyeD_Zpck2vBgfMPx6RcI6hI2CPtaaa491hg/exec",
basePath: "/shupz",
imageDir: "/images",
fallbackImage: "/shupz/images/logo.jpg",
businessName: "Shupz",
businessLogo: "/shupz/images/logo.jpg",
CACHE_KEY: "shupz_products_cache",
CART_KEY: "shupz_cart",
CACHE_TTL: 10 * 60 * 1000, // 10 minutes
WHATSAPP_NUMBER: "27724992664",
...
{ id: "dishwashingliquid5l", name: "Dishwashing Liquid 5L", price: 75.00, category: "dishwashing-liquid", niche: "cleaning", location: "lenasia", description: "Bulk 5-liter container of powerful dishwashing liquid. Great value!", badge: "🔥 Best Seller", image: "/shupz/images/dishwashingliquid5l.jpg" },
{ id: "laundrydetergent5l", name: "Laundry Detergent 5L", price: 95.00, category: "laundry-cleaning", niche: "cleaning", location: "lenasia", description: "Premium 5-liter laundry detergent for fresh, clean clothes.", badge: "🔥 Best Seller", image: "/shupz/images/laundrydetergent5l.jpg" },
{ id: "tilecleaner5l", name: "Tile Cleaner 5L", price: 85.00, category: "laundry-cleaning", niche: "cleaning", location: "lenasia", description: "Professional-grade tile cleaner. 5-liter container for sparkling floors.", badge: "✨ Value Pack", image: "/shupz/images/tilecleaner5l.jpg" }
];
// 🌐 State management
let PRODUCTS = [];
...
console.log('⚡ Loaded Shupz products from cache (instant)');
...
return PRODUCTS;
     ...
     if (!CONFIG.SHEETS_API_URL || CONFIG.SHEETS_API_URL === "" || CONFIG.SHEETS_API_URL.includes("YOUR_SHUPZ_DEPLOYMENT_ID")) {
...
if (!CONFIG.SHEETS_API_URL || CONFIG.SHEETS_API_URL === "" || CONFIG.SHEETS_API_URL.includes("YOUR_SHUPZ_DEPLOYMENT_ID")) return;
try {
...
if (!productsArray) return;
     ...
     if (lastRawSnapshot === null) lastRawSnapshot = JSON.stringify(window.SHUPZ_INLINE_PRODUCTS || window.SHUPZ_PRODUCTS || []);
...
niche: (product.niche || "cleaning").trim(),
location: (product.location || "lenasia").trim(),
...
nicheSlug: (product.niche || "cleaning").trim().toLowerCase().replace(/\s+/g, '-'),
locationSlug: (product.location || "lenasia").trim().toLowerCase().replace(/\s+/g, '-')
};
PRODUCTS_MAP.set(processed.id, processed);
return processed;
});
 // ✅ CRITICAL FIX: Expose to window.SHUPZ_PRODUCTS so index.html can render them!
 window.SHUPZ_PRODUCTS = PRODUCTS;
 window.SHUPZ_DATA = PRODUCTS;
 window.HIVETIMES_SHUPZ_PRODUCTS = PRODUCTS;
 return PRODUCTS;
}
...
this.save();
this.updateUI();
this.dispatchEvent('shupz:cart:updated', { cart: this.items, addedProduct: product });
return true;
},
...
this.save();
this.updateUI();
this.dispatchEvent('shupz:cart:updated', { cart: this.items });
return true;
},
...
return `<article class="product-card" data-id="${p.id}" data-category="${p.categorySlug}" data-price="${p.price}" data-name="${p.name}" data-description="${p.description}" data-image="${p.image}" onclick="openModal('${p.id}')" role="button" tabindex="0" style="cursor:pointer;"> <div class="product-image-wrap"> <img src="${p.image}" alt="${p.name}" loading="lazy" decoding="async" class="product-image" onerror="this.src='${p.imageFallback}'"> ${p.badge ? `<span class="product-badge">${p.badge}</span>` : ''} </div> <div class="product-info"> <h3 class="product-name">${p.name}</h3> <p class="product-description">${p.description}</p> <div class="product-price">${priceDisplay}</div> <button class="add-to-cart-btn" onclick="event.stopPropagation(); ShupzProducts.addToCart('${p.id}'); return false;"> ${btnText} </button> </div> </article>`;
},
...
else document.dispatchEvent(new CustomEvent('shupz:modal:open', { detail: { productId } }));
},
getStatus: () => ({
...
window.ShupzCart = Cart;
// 🚀 INITIALIZATION
(async function init() {
Cart.init();
 // ✅ FIXED: Check for both SHUPZ_INLINE_PRODUCTS and SHUPZ_PRODUCTS
  const inlineData = window.SHUPZ_INLINE_PRODUCTS || window.SHUPZ_PRODUCTS;
  ...
  if (hasInlineData) {
      ...
      try {
          document.dispatchEvent(new CustomEvent('shupz:products:loaded', { detail: { products: PRODUCTS } }));
      } catch (err) {}
      setTimeout(() => backgroundRefresh(window.SHUPZ_INLINE_PRODUCTS_HASH), 1500);
  } else {
      await fetchProducts();
      try {
          document.dispatchEvent(new CustomEvent('shupz:products:loaded', { detail: { products: PRODUCTS } }));
  ...
  console.group('🧴 Shupz Products Initialized');
...
const productList = {
 ...
 "@type": "ItemList",
 "name": "Shupz Product Catalog",
 "description": "Premium cleaning products from Shupz.",
 ...
 "image": product.image.startsWith('http') ? product.image : `https://shupz.co.za/${product.image}`,
 "sku": product.id,
 "brand": { "@type": "Brand", "name": "Shupz" },
 "offers": {
 "@type": "Offer",
 "priceCurrency": "ZAR",
 ...
 "seller": { "@type": "Organization", "name": "Shupz" }
}
}
}))
};
 let schemaEl = document.getElementById('shupzProductSchema');
 if (!schemaEl) {
     schemaEl = document.createElement('script');
     schemaEl.type = 'application/ld+json';
     schemaEl.id = 'shupzProductSchema';
     document.head.appendChild(schemaEl);
 }
 schemaEl.textContent = JSON.stringify(productList);
}
document.addEventListener('shupz:products:loaded', () => setTimeout(generateProductSchema, 500));
